...
architecture Timer_ARCH of Timer is
	...
	signal cntValue:		std_logic_vector(CNTVALUE_SIZE downto 0) := (others => '0');
...
begin
	...
	CNT: process(clock, controlSig, arrSig, stateSig)
	begin
		if (rising_edge(clock)) then
			if (controlSig(1) = '1') then -- Clear
				cntValue <= std_logic_vector(to_unsigned(0, cntValue'length));
				stateSig(0) <= '0';
			elsif (controlSig(0) = '1') then -- Enable
				if ((unsigned(cntValue) + 1) = unsigned(arrSig)) then -- Counter reaches arrSig
					stateSig(0) <= '1';
					...
					cntValue <= std_logic_vector(unsigned(cntValue) + 1);
				end if;
			end if;
		end if;
	end process CNT;
